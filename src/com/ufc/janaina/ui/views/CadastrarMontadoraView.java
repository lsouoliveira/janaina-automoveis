...
public class CadastrarMontadoraView extends View {
	 ...
	 */
	public CadastrarMontadoraView(Montadora montadora) {
		this();
		this.montadora = montadora;
		
		// Se o objetivo for editar o veículo
		// Atualizamos os campos com os valores do veículo passado
		if(montadora.getId() != -1) {
			// Altera o botão de salvar para "Atualizar"
			salvarBtn.setText("Atualizar");
			
			bairro.setText(montadora.getBairro());
			cidade.setText(montadora.getCidade());
			cnpj.setText(montadora.getCnpj());
			endereco.setText(montadora.getEndereco());
			nome.setText(montadora.getNome());
			numero.setText(String.valueOf(montadora.getNumero()));
			telefone.setText(montadora.getTelefone());
			uf.setSelectedItem(montadora.getUf());
	...
	*/
	public CadastrarMontadoraView() {
		...
		panel.add(lblCnpj, "cell 0 0,alignx trailing,aligny center");
		
		cnpj = new JTextField();
		panel.add(cnpj, "cell 1 0");
		cnpj.setColumns(10);
		cnpj.setMinimumSize(new Dimension(150, 19));
	...
	private void atualizar(ActionEvent e) {
		...
		v.setId(montadora.getId());
		
		montadoraDAO.update(v);
